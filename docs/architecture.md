...
| pfSense | Firewall / Router | bare metal on protectli mini PC | pfSense |
...
|Lab Infrastructure Network (VLAN 1)| `192.168.200.0/24` | Main lab subnet, managed by pfSense |
|Clinets (VLAN 10) | `192.168.10.0/24` | Isolated subnet for intentionally vulnerable machines |
|PBX Phone Server(VLAN 20)| `192.168.20.0/24` | Isolated subnet for VoIP traffic |
| IoT Vlan | `192.168.30.0/24` | Isolated subnet for possibly vulnerable IoT Devices.

---

## Routing and Firewall
The pfSense VM serves as the central router and firewall for all VLANS. Inter-VLAN routing is handled by pfSense, with firewall rules controlling traffic flow between segments. Firewall rules are configued to controll traffic between the lab and home networks. Full firewall ruleset documentation lives in `network/dns-filtering.md` as it relates to DNS traffic.
